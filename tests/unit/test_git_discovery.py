...
class TestGitDiscovery:
    ...
    def test_init_not_in_git_repo(self, tmp_path: Path) -> None:
        ...
        with patch("stomper.discovery.git.Repo", side_effect=InvalidGitRepositoryError):
    ...
    def test_get_file_status_modified(self, tmp_path: Path) -> None:
        ...
        # First call (staged) returns empty, second call (modified) returns the item
        mock_repo.index.diff.side_effect = [[], [mock_item]]
...
class TestDiscoverGitFiles:
    ...
    def test_discover_git_files_git_error(self, tmp_path: Path) -> None:
        ...
        with patch("stomper.discovery.git.GitDiscovery", side_effect=GitError("Git error")):
...
class TestPrintGitSummary:
    ...
    def test_print_git_summary_no_files(self) -> None:
        ...
        with patch("stomper.discovery.git.console") as mock_console:
            ...
            mock_console.print.assert_called_with("[yellow]No git-tracked files found to process[/yellow]")
